...
import { useUpdateRetailMutation } from '@/store/retailsApi'
...
const EditRetail = ({ setVisible, visible, retailId, defaultStoreName, defaultLocation, defaultContact }: DialogsProps) => {
  const [updateRetail, { isLoading }] = useUpdateRetailMutation()
  ...
  const handleSubmit = async () => {
    if (!validate()) return
    try {
      await updateRetail({ shop_id: retailId, store_name: storeName, location, contact })
      Toast.show({ type: 'success', text1: 'Retail shop updated successfully.' })
      setVisible(false)
    } catch (error: any) {
  ...
  return (
    <Dialogs onVisible={setVisible} visible={visible} title="Edit Retail Shop" subtitle="Update shop details">
